...
from inference.config.manager import load_config
from inference.config.models import RootConfig, TaskConfig, MetricConfig
...
class LLMEvaluator:
    ...
    def __init__(self, config_path: str):
        ...
        self.analysis = AnalysisManager(self.config.analysis)
        self.models = self._load_models()
        self.tasks = self._load_tasks()

    def _load_models(self) -> List[CoreModelInterface]:
    ...
    def _load_tasks(self) -> List[TaskConfig]:
        """Return task configs as a list of TaskConfig objects."""
    ...
    async def run_async(self) -> Dict[str, Any]:
        ...
        self.monitor = Monitor(
            enabled=evaluation.log_progress,
            level=evaluation.log_level,
        )
    ...
    async def _evaluate_single(
        self, model: CoreModelInterface, task: TaskConfig, run_dir: Path
    ) -> Dict[str, Any]:
        ...
        dataset = TaskProcessor.load_dataset(
            task.dataset,
            split=task.split,
            cache_dir=self.config.datasets_dir,
        )
        sample_size = task.sample_size or evaluation.sample_size
        if sample_size:
            dataset = dataset.select(range(min(sample_size, len(dataset))))

        adapter_name = task.task_adapter or "generic"
        task_adapter = task_adapters.create(adapter_name)

        prompts = task_adapter.build_prompts(dataset, task)
        metrics_cfgs = task.metrics
        has_metrics = bool(metrics_cfgs)
        has_reference_spec = task.reference_field is not None
        if has_metrics or (evaluation.save_detailed and has_reference_spec):
        ...
        checkpoint_batches = evaluation.checkpoint_batches or 0
        ...
        if checkpoint_batches and checkpoint_batches > 0:
            ...
            def _batch_cb(
                ...
                if (
                    ...
                    or batch_index % checkpoint_batches == 0
                ):
                    _flush_checkpoint()
        else:
            _batch_cb = None
        ...
        log_progress = evaluation.log_progress
        progress_every = evaluation.progress_every_batches
        ...
        (
            ...
            raw_predictions,
            extras,
        ) = await task_adapter.generate_predictions(
            model,
            prompts,
            task,
        ...
        postprocessed_predictions = task_adapter.postprocess_predictions(
        ...
        for metric_cfg in metrics_cfgs:
            if isinstance(metric_cfg, str):
                ...
                metric_args = metric_cfg.args
            metric_adapter = metric_adapters.create(
                metric_cfg.adapter if isinstance(metric_cfg, MetricConfig) else "hf"
            )
            metrics.update(
                metric_adapter.compute(
                    metric_predictions, metric_references, metric_name, metric_args
                )
            )
        prediction_sample_size = evaluation.prediction_sample_size
        ...
        if evaluation.save_detailed:
            max_examples = evaluation.max_detailed_examples
            total = len(prompts)
            limit = total if max_examples is None else min(total, max_examples)
            if extras is None:
                extras = task_adapter.collect_extras(task, limit) or []
            thinking_cfg = task.thinking_delimiters or {}
            detailed_examples = []
            for idx in range(limit):
                ...
                if thinking_cfg:
                    ...
                    thinking, answer_from_thinking = (
                        TaskProcessor.apply_thinking_delimiters(
